    ...
    public override void _PhysicsProcess(double delta)
	{
		...
		if(Input.IsActionJustPressed("launch rope")) {
			...
			if(mousePosition.X > 0){
				hookRaycast.Rotation = Mathf.Atan( mousePosition.Y / mousePosition.X);
			}else{
				hookRaycast.Rotation = Mathf.Atan( mousePosition.Y / mousePosition.X) + Mathf.Pi;
			}
		...
		// rope loosening
		if(Input.IsActionPressed("down") && distance >= currentRadius -1 && hooked && currentRadius <= maximumRadius){
			currentRadius += climbingSpeed;
